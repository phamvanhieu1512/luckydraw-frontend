...
useEffect(() => {
 ...
 const handleOpenBox = async () => {
    ...
    try {
      ...
      // Chuyển BigNumber sang bigint để gửi
      ...
      // parse logs để lấy event SpinResult
      let spinEvent = null;
      for (const log of receipt.logs) {
        try {
          ...
          // log không match event → bỏ qua
        }
      }

      if (!spinEvent) {
        console.error("Không tìm thấy event SpinResult trong logs!");
        return;
      }
      ...
      if (rewardType === "token") {
        newReward = {
          type: "token",
          amount: ethers.formatEther(ethers.toBigInt(amount)),
        };
      ...
      console.log("Bạn nhận được:", newReward);
      setReward(newReward);
      setOpen(true);
...
//   try {
//     const price = await contract.getPrice();
//     console.log("Box Price:", price.toString());
...
//     let newReward;
//     let rewardData = { type: rewardType };
//     if (rewardType === "token") { 
//       rewardData.amount = ethers.formatEther(amount);
//       newReward = { type: "token", amount: ethers.formatEther(amount) };
//       setShowConfetti(true);
//     }
//     if (rewardType === "nft") {
//       rewardData.nftId = nftId;
//       newReward = { type: "nft", nftId };
//       setShowConfetti(true);
//     }
//     if (rewardType === "none") {
//       // alert("Chúc bạn lần sau")
//       newReward = { type: "none", message: "Chúc bạn lần sau!" };
//     }

//     console.log("Bạn nhận được:", rewardData);
//     setReward(newReward);
//     setOpen(true);
//   } catch (err) {
//     console.error("Lỗi khi mở hộp:", err);
